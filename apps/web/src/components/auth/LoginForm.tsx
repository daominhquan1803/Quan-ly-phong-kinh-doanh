    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        ...
        <input
          type="email"
          {...register("email")}
          className="w-full rounded-md border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-navy-900"
          placeholder="ban@hoanggia.local"
        />
        {errors.email && <p className="text-xs text-brandRed-600 mt-1">{errors.email.message}</p>}
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-900 mb-1">Mật khẩu</label>
        <input
          type="password"
          {...register("password")}
          className="w-full rounded-md border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-navy-900"
