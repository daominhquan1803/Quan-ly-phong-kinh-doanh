...
const PREV_MONTH_COLORS = ["#0B2447", "#2F6FED", "#D4A017"];
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-5">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-1">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-medium text-gray-900">Tình hình lên đơn hàng trong tháng</h2>
        </div>
        <div className="flex items-center gap-2">
      ...
      {isLoading && <p className="text-sm text-gray-500 py-16 text-center">Đang tải...</p>}
      {!isLoading && data && (
        <ResponsiveContainer width="100%" height={320}>
          <LineChart data={data.days} margin={{ top: 5, right: 16, left: 0, bottom: 4 }}>
            ...
            <XAxis
              dataKey="day"
              tick={{ fontSize: 12 }}
              label={{ value: "Ngày trong tháng", position: "insideBottom", offset: -4, fontSize: 11, fill: "#9CA3AF" }}
            />
            <YAxis tickFormatter={(v) => `${(v / 1_000_000).toFixed(0)}tr`} tick={{ fontSize: 12 }} width={48} />
            <Tooltip formatter={(v: number) => formatCurrencyVND(v)} labelFormatter={(d) => `Ngày ${d}`} />
            <Legend wrapperStyle={{ fontSize: 12 }} />
            <ReferenceLine
              x={data.todayDate}
              stroke="#9CA3AF"
              strokeDasharray="4 4"
              label={{ value: "Hôm nay", fontSize: 10, fill: "#9CA3AF", position: "top" }}
            />
            {months.map((m, idx) => (
              <Line
                key={m.label}
