...
export async function POST() {
  try {
    ...
    const res = await fetch(`${workerUrl}/sync-quotes`, {
      method: "POST",
    ...
    console.error("quotes/sync POST error", err);
    return NextResponse.json(
      { error: "Không gọi được service đồng bộ Báo giá. Kiểm tra worker có đang chạy không." },
      { status: 502 }
    );
  }
}
